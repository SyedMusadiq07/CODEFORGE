...
const ProblemPage = () => {
  ...
  useEffect(() => {
    if (submission?.status?.description === "Compilation Error" || 
        submission?.status?.description === "Runtime Error") {
      setErrorMessage(submission?.compile_output || submission?.stderr || "Execution error occurred");
    } else {
      setErrorMessage("");
    }
  }, [submission]);
        ...
        return (
          <div className="p-4 space-y-4">
            ...
            <div className="flex justify-center mb-4">
              <HintButton problemId={id} />
            </div>

            {/* Static hints from database */}
            {problem?.hints ? (
              <div className="bg-base-200 p-6 rounded-xl">
                <h4 className="font-semibold mb-2 text-base-content/70">Static Hint:</h4>
                <span className="bg-black/90 px-4 py-1 rounded-lg font-semibold text-white text-lg">
                  {problem.hints}
                </span>
              </div>
            ) : (
              <div className="text-center text-base-content/70">No static hints available</div>
            )}
          </div>
        );
      default:
    ...
    <div className="min-h-screen bg-gradient-to-br from-base-300 to-base-200">
      <nav className="navbar bg-base-100 shadow-lg px-4">
        ...
        <div className="flex-none gap-2">
          {/* AI Features */}
          <ExplainSolutionModal problemId={id} />
          {authUser && <AIRecommendations userId={authUser._id} />}
              ...
              <div className="p-4 border-t border-base-300 bg-base-200">
                <div className="flex justify-between items-center">
                  <div className="flex gap-2">
                    ...
                    {/* Debug Button - only show if there's an error */}
                    {errorMessage && (
                      <DebugButton 
                        problemId={id}
                        code={code}
